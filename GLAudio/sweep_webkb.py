...
if NAME == "wisconsin":
    PROJECT_NAME = "sweep-wisconsin"
    OUTPUT_DIM = 5
elif NAME == "texas":
    PROJECT_NAME = "sweep-texas"
    OUTPUT_DIM = 5
elif NAME == "cornell":
    PROJECT_NAME = "sweep-cornell"
    OUTPUT_DIM = 5
else:
...
if __name__ == "__main__":
    ...
    sweep_configuration = {
        ...
        "parameters": {
            "N": {"values": [1, 2, 4, 6, 8, 10]},
            "L": {"values": [1, 2, 3, 5, 8, 10]},
            "h": {"values": [0.01, 0.02, 0.05, 0.1, 0.2, 0.5, 1]},
            "hidden_dim": {"values": [8, 16, 24, 32]},
            "lr": {"values": [1e-3, 2.5e-3, 5e-3, 1e-2]},
            "weight_decay": {"values": [0, 1e-3, 2.5e-3, 5e-3, 1e-2]},
    ...
    wandb.agent(sweep_id=sweep_id, function=main, count=600)
